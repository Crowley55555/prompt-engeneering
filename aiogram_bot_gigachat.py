"""
Телеграм-бот для создания SEO-оптимизированных описаний товаров с GigaChat API (aiogram)

НАСТРОЙКА:
1. Установите зависимости: pip install -r requirements.txt
2. Создайте .env файл с переменными:
   GIGACHAT_CREDENTIALS=путь_к_файлу_с_учетными_данными
...
4. Настройте GigaChat:
   ...
   - Создайте приложение и получите учетные данные
   - Сохраните их в JSON файл

МОНИТОРИНГ:
- Все запросы к GigaChat автоматически отслеживаются в Langfuse
...
class SEOAssistantBotGigaChat:
    def __init__(self):
        # Загрузка учетных данных GigaChat
        if not GIGACHAT_CREDENTIALS or not os.path.exists(GIGACHAT_CREDENTIALS):
            raise ValueError("Файл с учетными данными GigaChat не найден!")
        
        with open(GIGACHAT_CREDENTIALS, 'r', encoding='utf-8') as f:
...
seo_bot = SEOAssistantBotGigaChat()
...
Я - SEO-ассистент для создания описаний товаров на базе GigaChat. Я помогу тебе создать профессиональные, SEO-оптимизированные описания для любых товаров.
...
async def handle_description_request(message: types.Message, state: FSMContext):
    ...
    processing_msg = await message.answer("🔄 Обрабатываю запрос через GigaChat...")
...
async def main():
    ...
    if not GIGACHAT_CREDENTIALS:
        logger.error("GIGACHAT_CREDENTIALS не найден в переменных окружения!")
        return
    
    try:
        logger.info("🚀 Запуск SEO-ассистента бота с GigaChat на aiogram...")
